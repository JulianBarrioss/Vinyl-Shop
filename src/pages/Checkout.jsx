...
const Checkout = () => {
  ...
  const proceedPurchase = () => {
    ...
    if (auth.currentUser) {
      settext(state.total ? "Thanks for your purchase" : "Your cart is empty!");
    ...
    <>
      ...
      <div className="checkout">
        <div className="checkout__products-container">
          ...
          {state.cart.length === 0 && (
            <p className="checkout__empty-cart">
              your cart is empty, <a href="/">go back to add something...</a>
            </p>
          )}
        ...
        <div className="checkout__amount">
          <div className="checkout__amount-container">
            ...
            <button
              type="button"
              className="checkout__proceed-button"
              onClick={proceedPurchase}
            >
              Proceed
            </button>
          </div>
          <div className={proceededClass ? "proceeded" : "proceed"}>{text}</div>
        </div>
      </div>
    </>
  );
